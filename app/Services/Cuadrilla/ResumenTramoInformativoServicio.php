...
class ResumenTramoInformativoServicio
{
    ...
    protected function filasSueldo(CuaGrupo $grupo, CuadTramoLaboral $tramoLaboral, string $fechaCorte, bool $acumula): array
    {
        ...
        $pendiente = $this->calcularDeudaPendienteReal($fuenteRegistros, $fechaCorte, 'total_pago_jornal');
    ...
    protected function filasSueldoMensual(CuaGrupo $grupo, CuadTramoLaboral $tramoLaboral, string $fechaCorte, bool $acumula, Collection $registrosDeEsteTramo): array
    {
        ...
        if ($acumula) {
            $mesesConPendienteHistorico = CuadRegistroDiario::where('codigo_grupo', $grupo->codigo)
                ...
                ->filter(fn($regs) => $this->calcularDeudaPendienteReal($regs, $fechaCorte, 'total_pago_jornal') > 0)
                ->keys();
        }
    ...
    protected function filaBono(CuaGrupo $grupo, CuadTramoLaboral $tramoLaboral, string $fechaCorteBono, bool $acumula): ?array
    {
        ...
        // Se evalúan los pagos asociados a los bonos del rango hasta la fecha de corte del bono
        $montoPagado = $this->sumarMontoPagadoEnTramo($fuenteRegistros, $tramoLaboral, 'total_bono');
        $ultimoPago = $this->ultimoPagoValidoEnTramo($fuenteRegistros, $tramoLaboral);
        
        // La deuda pendiente real considera si el registro YA TIENE un desglose asociado (incluso si la fecha del desglose es el sábado)
        $pendiente = $this->calcularDeudaPendienteReal($fuenteRegistros, $fechaCorteBono, 'total_bono');
    ...
    protected function filasAdicionales(CuaGrupo $grupo, CuadTramoLaboral $tramoLaboral, string $fechaCorte, bool $acumula): array
    {
        ...
        return $descripciones->map(function ($descripcion) use ($grupo, $tramoLaboral, $fechaCorte, $acumula, $gastosDeEsteTramo, $gastosHistoricos) {
            ...
            $pendiente = $this->calcularDeudaPendienteReal($fuentePendiente, $fechaCorte, 'monto');

            if ($deudaActual <= 0 && $pendiente <= 0 && $montoPagado <= 0) {
                return null;
            }
            ...
            return [
                ...
                'condicion' => $pendiente <= 0 ? 'Pagado' : 'Pendiente',
    ...
    /**
     * Calcula la deuda pendiente real acumulada a la fecha de corte.
     * Si el registro YA fue pagado (tiene esta_pagado = true), la deuda pasa a 0.00.
     */
    protected function calcularDeudaPendienteReal(Collection $registros, string $fechaCorte, string $campoMonto): float
    {
        return $registros
            ->filter(function ($r) {
                // Si el registro ya fue marcado como pagado por cualquier desglose registrado, ya no es deuda pendiente
                return !$r->esta_pagado;
            })
            ->sum($campoMonto);
    }

    /**
     * Suma los montos pagados de los registros de este tramo cuya FECHA DE DESGLOSE 
     * caiga dentro del tramo laboral evaluado (ej: el día sábado del pago).
     */
    protected function sumarMontoPagadoEnTramo(?Collection $registros, CuadTramoLaboral $tramoLaboral, string $campoMonto): float
    {
        ...
        return (float) $registros
            ->filter(function ($r) use ($tramoLaboral) {
                if (!$r->esta_pagado || !$r->desgloseDetalle?->desglose?->fecha) {
                    return false;
                }

                $fechaDesglose = $r->desgloseDetalle->desglose->fecha->format('Y-m-d');

                // Válido si la fecha en que se emitió el Desglose cae dentro del rango del tramo laboral
                return $fechaDesglose >= $tramoLaboral->fecha_inicio && $fechaDesglose <= $tramoLaboral->fecha_fin;
            })
            ->sum($campoMonto);
    }

    /**
     * Obtiene el comprobante o registro del desglose pagado durante este tramo laboral.
     */
