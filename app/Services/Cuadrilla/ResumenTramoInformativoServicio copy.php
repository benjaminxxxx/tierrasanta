...
class ResumenTramoInformativoServiciocopy
{
    ...
    {
        // Agregado 'actividadesBonos' al eager loading para calcular correctamente el total con bonos
        ...
        // CORRECCIÓN: Se usa total_pago_jornal para incluir los bonos que se pagan con el jornal
        ...
        $ultimoPago = $this->ultimoPagoValidoComoDeCorte($registrosDeEsteTramo, $fechaCorte);
        $montoPagado = $this->sumarMontoPagadoValido($fuenteRegistros, $fechaCorte, 'total_pago_jornal');
    ...
    protected function filasSueldoMensual(CuaGrupo $grupo, CuadTramoLaboral $tramoLaboral, string $fechaCorte, bool $acumula, Collection $registrosDeEsteTramo): array
    {
        ...
        return $todosLosMeses->map(function ($mesKey) use ($grupo, $fechaCorte, $acumula, $actualPorMes) {
            $nombreMes = $this->meses[(int) Carbon::parse($mesKey . '-01')->format('n')];
            $registrosDeEsteTramoDelMes = $actualPorMes->get($mesKey, collect());

            // CORRECCIÓN: Se usa total_pago_jornal
            ...
            $ultimoPago = $this->ultimoPagoValidoComoDeCorte($registrosDeEsteTramoDelMes, $fechaCorte);
            $montoPagado = $this->sumarMontoPagadoValido($fuenteRegistros, $fechaCorte, 'total_pago_jornal');
    ...
    {
        // Permanece igual: Filtra solo bonos que NO se pagan con el jornal
        ...
        $ultimoPago = $this->ultimoPagoValidoComoDeCorte($registrosDeEsteTramo, $fechaCorteBono);
        $montoPagado = $this->sumarMontoPagadoValido($fuenteRegistros, $fechaCorteBono, 'total_bono');
    ...
    protected function filasAdicionales(CuaGrupo $grupo, CuadTramoLaboral $tramoLaboral, string $fechaCorte, bool $acumula): array
    {
        ...
        return $descripciones->map(function ($descripcion) use ($grupo, $fechaCorte, $acumula, $gastosDeEsteTramo, $gastosHistoricos) {
            ...
            $ultimoPago = $this->ultimoPagoValidoComoDeCorte($delTramo, $fechaCorte);
            $montoPagado = $this->sumarMontoPagadoValido($fuentePendiente, $fechaCorte, 'monto');
    ...
    protected function ultimoPagoValidoComoDeCorte(Collection $registros, string $fechaCorte)
    {
        return $registros
            ->filter(fn($r) => $r->esta_pagado && $r->desgloseDetalle?->desglose && $r->desgloseDetalle->desglose->fecha->lte($fechaCorte))
    ...
    protected function sumarMontoPagadoValido(Collection $registros, string $fechaCorte, string $campoMonto): float
    {
        return $registros
            ->filter(function ($r) use ($fechaCorte) {
