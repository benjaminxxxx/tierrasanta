...
class InsumoKardexMovimientosServicio
{
    ...
    private function crearMovimientoSaldoInicial(InsKardex $insumoKardex): void
    {
        if ($insumoKardex->stock_inicial > 0) {
            InsKardexMovimiento::create([
                ...
                'salida_costo_unitario' => round($this->costoTotalAcumulado / $this->stockAcumulado, 13), // Usa el costo inicial como costo unitario en el saldo
                'salida_costo_total' => $this->costoTotalAcumulado,
            ]);
        }
    }
    ...
    private function procesarEntrada(InsKardex $kardex, CompraProducto $compra): void
    {
        ...
        InsKardexMovimiento::create([
            ...
            'salida_cantidad' => round($this->stockAcumulado, 3),
            'salida_costo_unitario' => round($this->calcularCostoUnitarioPromedio(), 13),
            'salida_costo_total' => round($this->costoTotalAcumulado, 13),
        ]);
    }
    ...
    private function procesarSalida(InsKardex $kardex, AlmacenProductoSalida $salida): void
    {
        ...
        InsKardexMovimiento::create([
            ...
            // Saldo final después del movimiento
            'entrada_cantidad' => round($this->stockAcumulado, 3), // La cantidad restante es la entrada del saldo final
            'entrada_costo_unitario' => round($this->calcularCostoUnitarioPromedio(), 13),
            'entrada_costo_total' => round($this->costoTotalAcumulado, 13),
        ]);
