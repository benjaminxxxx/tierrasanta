            ...
            <x-flex>
                        ...
                        <x-slot name="tbody">
                            @foreach ($historial as $historia)
                                <x-tr class="{{$historia->id==$salidaId?'!bg-lime-600':''}}">
                                    <x-td class="text-center">{{ $historia->campo_nombre }}</x-td>
                                    <x-td class="text-center">{{ $historia->cantidad }}</x-td>
                                    <x-td class="text-center">{{ $historia->fecha_reporte }}</x-td>
                                    <x-td class="text-right">{{ $historia->costo_por_kg }}</x-td>
                                    <x-td class="text-right">{{ $historia->total_costo }}</x-td>
                                </x-tr>
                            @endforeach
                            <x-tr>
                                <x-th></x-th>
                ...
                @if ($entrada)
                    <div class="w-[14rem]">
                        ...
                        <p class="text-center mt-2">{{ number_format($porcentajeRestante, 2) }}% restante ({{$restnteUnidad . $entrada->producto->unidad_medida}})</p>
                    </div>
                @endif
