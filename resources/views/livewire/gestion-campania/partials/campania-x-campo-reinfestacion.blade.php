<x-tr class="">
    <x-th-header colspan="2">RE-INFESTACIÓN</x-th-header>
    <x-td class="bg-gray-50 text-xs text-gray-700 uppercase  dark:bg-gray-700 dark:text-gray-400">
        ...
        @if ($campania)
            ...
            <x-button type="button" @click="$wire.dispatch('sincronizarReinformacionInfestacion',{campaniaId:{{ $campania->id }}})">
...
<x-tr>
    ...
    <x-td>{{ number_format($campania->reinfestacion_numero_pencas, 0) }}</x-td>
</x-tr>
<x-tr>
    <x-td>Kg totales de madres</x-td>
    <x-td>{{ number_format($campania->reinfestacion_kg_totales_madre, 0) }}</x-td>
</x-tr>
<x-tr>
    <x-td>Kg de madres para infestador cartón</x-td>
    <x-td>{{ number_format($campania->reinfestacion_kg_madre_infestador_carton, 0) }}</x-td>
</x-tr>
<x-tr>
    <x-td>Kg de madres para infestador tubos</x-td>
    <x-td>{{ number_format($campania->reinfestacion_kg_madre_infestador_tubos, 0) }}</x-td>
</x-tr>
<x-tr>
    <x-td>Kg de madres para infestador mallita</x-td>
    <x-td>{{ number_format($campania->reinfestacion_kg_madre_infestador_mallita, 0) }}</x-td>
</x-tr>
<x-tr>
