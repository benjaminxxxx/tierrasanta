<div class="space-y-6">
    ...
    <x-card>
        <div class="inline-flex p-1 rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
            <button
                type="button"
                wire:click="cambiarVista('Proyectada')"
                class="px-4 py-2 text-sm font-semibold rounded-md transition-all duration-150
                    {{ $vista === 'Proyectada'
    ...
    @if ($vista == 'Proyectada')
        <livewire:gestion-planilla.planilla-proyectada-component :mes="$mes" :anio="$anio" wire:key="cpm_{{ $mes }}_{{ $anio }}_{{ $vista }}" />
    @else
        <livewire:gestion-planilla.planilla-plame-component :mes="$mes" :anio="$anio" wire:key="cpm_{{ $mes }}_{{ $anio }}_{{ $vista }}" />
    @endif
