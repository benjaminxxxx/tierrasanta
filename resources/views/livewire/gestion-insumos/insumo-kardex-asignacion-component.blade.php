<div x-data="asignacionKardex">
    ...
    <div class="mt-5" x-show="ayudaActivada">
        <x-kardex-proceso :pasoActivo="2" :linkResumenBlanco="$linkResumenBlanco"
            :linkResumenNegro="$linkResumenNegro" />
    </div>
...
<script>
    Alpine.data('asignacionKardex', () => ({
        ...
        autoAsignarTodo(tipo) {
            const stockInicial = tipo === 'blanco'
                ? this.stockInicialBlanco
                : this.stockInicialNegro;

            // ── 1. Construir línea de tiempo unificada ─────────────────
            // Compras del tipo
            const entradasDelTipo = this.compras
                .filter(c => c.tipo_kardex === tipo)
                .map(c => ({ ...c, _esEntrada: true }));

            // Salidas ya asignadas a este tipo
            const salidasAsignadas = this.allSalidas
                .filter(s => this.resolveTipoSalida(s) === tipo)
                .map(s => ({ ...s, _esEntrada: false }));

            // Salidas sin asignar — candidatas
            const salidasPendientes = this.allSalidas
                .filter(s => !this.resolveTipoSalida(s))
                .map(s => ({ ...s, _esEntrada: false, _pendiente: true }));

            // Unificar y ordenar cronológicamente
            // Entradas antes que salidas si misma fecha
            const timeline = [...entradasDelTipo, ...salidasAsignadas, ...salidasPendientes]
                .sort((a, b) => {
                    const fechaA = new Date(a.fecha);
                    const fechaB = new Date(b.fecha);
                    if (fechaA - fechaB !== 0) return fechaA - fechaB;
                    // misma fecha: entradas primero
                    if (a._esEntrada && !b._esEntrada) return -1;
                    if (!a._esEntrada && b._esEntrada) return 1;
                    return 0;
                });

            // ── 2. Simular saldo corriente ─────────────────────────────
            let saldo = Math.round(stockInicial * 1000) / 1000;

            for (const evento of timeline) {
                if (evento._esEntrada) {
                    saldo += Number(evento.cantidad);
                    saldo = Math.round(saldo * 1000) / 1000;
                    continue;
                }

                if (!evento._pendiente) {
                    // Salida ya asignada — consumir saldo igual
                    saldo -= Number(evento.cantidad);
                    saldo = Math.round(saldo * 1000) / 1000;
                    continue;
                }

                // Salida pendiente — asignar solo si hay saldo suficiente
                const cantidad = Math.round(Number(evento.cantidad) * 1000) / 1000;
                const saldoRedondeado = Math.round(saldo * 1000) / 1000;

                if (cantidad <= saldoRedondeado) {
                    this.changes.set(evento.id, tipo);
                    saldo -= cantidad;
                    saldo = Math.round(saldo * 1000) / 1000;
                }
                // Si no alcanza el saldo, simplemente se salta — no rompe el loop
                // porque puede haber una salida posterior más pequeña que sí quepa
            }
        },
        autoAjustarCompra(compra) {
            ...
            // ── BUG 2: Stock inicial ────────────────────────────────────
            // Solo aplica a la primera compra del tipo (no hay compras anteriores)
            const hayCompraAnterior = this.compras.some(c =>
                c.tipo_kardex === tipo &&
                new Date(c.fecha) < fechaCompra
            );

            // Determinar desde qué fecha contar el stock disponible acumulado
            // Si es la primera compra → desde el inicio (stock inicial incluido)
            // Si no → desde la fecha de esta compra
            const fechaDesde = hayCompraAnterior ? fechaCompra : new Date('1900-01-01');

            // ── BUG 1: Sumar TODAS las compras del mismo tipo y misma fecha ──
            // (o desde el inicio si es la primera)
            let stockDisponible = hayCompraAnterior
                // Compras del mismo día y tipo (puede haber varias)
                ? this.compras
                    .filter(c =>
                        c.tipo_kardex === tipo &&
                        new Date(c.fecha).toDateString() === fechaCompra.toDateString()
                    )
                    .reduce((sum, c) => sum + Number(c.cantidad), 0)
                // Primera compra → sumar stock inicial + todas las compras hasta esta fecha inclusive
                : (() => {
                    const stockInicial = tipo === 'blanco'
                        ? this.stockInicialBlanco
                        : this.stockInicialNegro;

                    const stockComprasHastaHoy = this.compras
                        .filter(c =>
                            c.tipo_kardex === tipo &&
                            new Date(c.fecha) <= fechaCompra
                        )
                    ...
                    return stockInicial + stockComprasHastaHoy;
                })();

            // ── Restar salidas ya asignadas a este tipo desde fechaDesde ──
            // Incluye salidas con fecha anterior a la compra si hay stock inicial
            const salidasYaAsignadas = this.allSalidas.filter(s =>
                this.resolveTipoSalida(s) === tipo &&
                new Date(s.fecha) >= fechaDesde
            );

            salidasYaAsignadas.forEach(s => {
                stockDisponible -= Number(s.cantidad);
            });

            if (stockDisponible <= 0) return;

            // ── Salidas sin asignar desde fechaDesde, ordenadas por fecha ──
            const salidasDisponibles = this.allSalidas
                .filter(s =>
                    !this.resolveTipoSalida(s) &&
                    new Date(s.fecha) >= fechaDesde
                )
                .sort((a, b) => new Date(a.fecha) - new Date(b.fecha));

            // ── Asignar hasta agotar stock ─────────────────────────────
            for (const salida of salidasDisponibles) {
                if (stockDisponible <= 0) break;

                if (Number(salida.cantidad) <= stockDisponible) {
                    this.changes.set(salida.id, tipo);
                    stockDisponible -= Number(salida.cantidad);
                } else {
                    // No dividir salidas automáticamente
                    break;
                }
            }
        },
