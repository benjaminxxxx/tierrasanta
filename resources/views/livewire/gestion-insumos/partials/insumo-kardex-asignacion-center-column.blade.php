<div class="flex flex-col h-full">
    {{-- Header --}}
    <div class="bg-slate-100 dark:bg-slate-800 rounded-t-lg border border-b-0 border-slate-200 dark:border-slate-700 p-4">
    ...
    {{-- Body --}}
    <div class="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-b-lg overflow-y-auto p-4 space-y-3">
        
        {{-- Estado vacío --}}
        <template x-if="sinAsignar.length === 0">
            <div class="flex items-center justify-center h-32 text-slate-500 dark:text-slate-400">
                <p class="text-sm">
        ...
        <template x-for="salida in sinAsignar" :key="salida.id">
            <div 
                @click="selectOne(salida, $event.shiftKey)"
            ...
            >
                <div class="flex items-start gap-3">
                    ...
                    <input 
                        ...
                        :checked="selected.has(salida.id)"
                        @click.stop 
                        @change="selectOne(salida, false)" 
                    />
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1" x-text="salida.fecha"></p>
                    </div>

                    <div class="text-right">
                        <div class="text-lg font-bold text-slate-800 dark:text-slate-100" x-text="salida.cantidad.toFixed(3)"></div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">
                            <span x-text="salida.unidad_medida"></span>
                        </div>
                    </div>
                </div>
            </div>
