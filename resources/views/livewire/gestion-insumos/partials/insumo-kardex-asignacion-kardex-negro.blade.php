<div
    class="rounded-xl border-2 border-slate-600 bg-slate-900 bg-opacity-40 shadow-lg shadow-slate-600/10
           overflow-hidden transition-all duration-300 flex flex-col h-full">
    @if ($kardexNegro)
        {{-- Header --}}
        <div class="px-6 py-4 border-b-2 border-slate-600 bg-slate-800 bg-opacity-40">
                ...
                <div>
                    <x-flex class="justify-between pb-3">
                        ...
                        <template x-if="hasSelectedIn('negro')">
                            <button @click="quitarSeleccionados('negro')"
                    ...
                    <div class="grid grid-cols-4 gap-2 text-xs">
                        <x-kardex-stat label="Stock Inicial" color="blue">
                            <span x-text="stockInicialNegro.toFixed(1) + ' kg'"></span>
                        </x-kardex-stat>

                        <x-kardex-stat label="Compras" color="blue">
                            <span x-text="totalComprasNegro.toFixed(1) + ' kg'"></span>
                        </x-kardex-stat>

                        <x-kardex-stat label="Salidas" color="blue">
                            <span x-text="totalSalidasNegro.toFixed(1) + ' kg'"></span>
                        </x-kardex-stat>

                        <x-kardex-balance>
                            <span x-text="balanceNegro.toFixed(1) + ' kg'"
                                :class="balanceNegro < 0 ? 'text-red-400' : 'text-emerald-400'"></span>
                        </x-kardex-balance>
                    </div>
               ...
               hover:border-blue-600 hover:bg-opacity-40"
                            ...
                            <div class="flex gap-2 transition-all"
                                ...
                                <x-button @click="moverCompra(item)" variant="secondary">
                                    ...
                                    <span
                                        x-text="item.tipo_kardex === 'negro' ? 'Pasar a Blanco' : 'Pasar a Negro'"></span>
                                </x-button>
                        ...
                        <div @click="selectOne(item, $event.shiftKey)" @contextmenu.prevent="selectOne(item.id, true)"
                            :class="{
                                ...
                                'bg-blue-800 bg-opacity-60 border-l-blue-300 border-2 border-blue-500 shadow-lg shadow-blue-500/30': selected
                                    .has(item.id) && item.tipo_kardex === 'negro',
                                ...
                                'bg-blue-900 bg-opacity-50 border-l-blue-400 border border-blue-600 shadow-md shadow-blue-500/20':
                                    !selected.has(item.id) && changes.has(item.id) && item.tipo_kardex === 'negro',
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="text-right"
                                    :class="item.tipo_kardex === 'negro' ? 'text-slate-400' : 'text-blue-400'">
                                    <div class="text-lg font-bold" x-text="item.cantidad.toFixed(1)"></div>
                                    <div class="text-xs text-slate-500">kg</div>
                                </div>
                            </div>
    ...
    @else
        <x-warning>
            No hay Kardex Negro <x-button href="{{ route('gestion_insumos.kardex') }}">Crear Kardex</x-button>
        </x-warning>
    @endif
</div>
